...
3. **지연 로딩(Lazy Loading) 리소스 동기화**: `loading="lazy"` 속성이 부여된 이미지가 PDF 렌더링 시점에 누락되지 않도록, 해당 속성을 일괄 무효화한 뒤 DOM 트리에 존재하는 모든 `HTMLImageElement`의 `load` 이벤트 및 `complete` 상태를 교차 대기하여 네트워크 응답 완료를 보장한다.
